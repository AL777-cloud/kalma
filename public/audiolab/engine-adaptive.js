...
class AdaptiveEngine {
  ...
  _onMovementChange(from, to) {
    ...
    let voiceConfig;
    if (to === 'neutral') {
      // Subtle shimmer — high, airy, gentle
      const degree = scale[Math.floor(Math.random() * scale.length)];
      voiceConfig = {
        freq: base * Math.pow(2, (degree / 12) + 2),  // two octaves up
        types: ['sine', 'sine'],
        detune: [0, 6],
        filterFreq: 2000,
        volume: 0.04,
      ...
      // Brighter, more present + energetic beats
    ...
    // Smooth fade in — gain rises slowly, filter sweeps open
    gain.gain.setTargetAtTime(voiceConfig.volume, now, voiceConfig.attack);
    filter.frequency.setTargetAtTime(voiceConfig.filterFreq, now, voiceConfig.attack * 0.8);

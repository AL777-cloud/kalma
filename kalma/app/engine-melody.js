...
class MelodyEngine {
  ...
  // Pick a note that's consonant with the current chord
  // 65% chord tone, 25% other scale degree, 10% passing tone
  ...
  _generateRightHand(pattern, motif, chord, beats, beatMs, startTime, vel, cycle) {
    ...
    switch (pattern) {
      ...
      case 'sparse': {
        // Calm isolated piano notes — prefer chord tones for consonance
        const chordTones = chord.map(c => c % 12);
        for (let b = 0; b < beats; b++) {
          if (Math.random() > 0.45) continue;
          // 70% chance: pick a chord tone. 30%: pick a scale tone
          let degree;
          if (Math.random() < 0.7 && chordTones.length > 0) {
            degree = chordTones[Math.floor(Math.random() * chordTones.length)];
          } else {
          ...
          events.push({
            ...
            velocity: vel * (0.45 + Math.random() * 0.3),
